...
const Menu = ({ text, isRed, ...props }) =>
    <button {...props} className={`w-full text-left outline-none border-t px-3 text-lg font-thin border-[#C588BC] ${isRed && "text-red-500"}`}>{text}</button>
    ...
    return (
        <div className="relative">
            <div className="flex items-center">
                ...
                <Image onClick={() => setIsShopDropDown(!showDropdown)} className="z-40 cursor-pointer" src="/icons/chevron_down.svg" height={20} width={30}></Image>
            </div>
            {
                showDropdown && <div style={{ width: (width * 160.64 / 202) + 40, marginTop: -40, paddingTop: 30 }} className="rounded-xl right-0 glass absolute bg-white">
